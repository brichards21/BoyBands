...
tally(~ Bands$dance_speed, format = "perc")



gghistogram(Bands, x = "highest_pos", add = "mean", rug = TRUE, 
            color = "dance_speed", fill = "dance_speed",
...
```{r message = FALSE, warning = FALSE}
tally( ~ featuring_artist, data = Bands, format = "perc")
...
```{r, message=FALSE}
Bands$month <- factor(Bands$month ,levels = month.name)
ggplot(Bands, aes(x = month, fill = month)) + 
  geom_dotplot() + 
  theme_bw() + 
  theme(legend.position = "none") + 
...
```{r, message=FALSE}
ggplot(Bands, aes(x = as.factor(year), fill = as.factor(year))) + geom_dotplot() + 
  theme_bw() + 
  theme(legend.position = "none") + 
...
```{r}
Bands_months <- Bands[, c(6, 2)] %>%
  mutate(month = as.numeric((Bands$month)))
